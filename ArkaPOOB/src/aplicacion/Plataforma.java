...
/**
 * Representa una plataforma apodada Nave Espacial Vaus, que impide que una bola
 * salga de la zona de juego, haci�ndola rebotar.
 * 
	...
	public void moveRight(Jugador otroJugador) {
		if (x + getAncho() + VELOCIDAD < limXFinal) { // No se salga por la derecha
			if (x + getAncho() + VELOCIDAD > otroJugador.getX()
					&& x + getAncho() + VELOCIDAD < otroJugador.getX() + otroJugador.getAncho()) { // Si me estrello con
																									// la otra
																									// plataforma cuando
																									// me muevo hacia la
																									// izquierda
				if (((x - otroJugador.getAncho()) >= limXInicial) && ((otroJugador.getX() + getAncho()) <= limXFinal)) { // Al
																															// hacer
																															// swap
																															// no
																															// se
	...
	public void moveLeft(Jugador otroJugador) {
		if (x - VELOCIDAD > limXInicial) { // No se salga por la izquierda
			if (x - VELOCIDAD < otroJugador.getX() + otroJugador.getAncho() && x - VELOCIDAD > otroJugador.getX()) { // Si
																														...
																														// la
																														// izquierda
				if (((x - getAncho()) >= limXInicial) && ((x + otroJugador.getAncho()) <= limXFinal)) { // Al hacer swap
																										// no se salga
																										// ninguna
																										// plataforma
																										// por los
