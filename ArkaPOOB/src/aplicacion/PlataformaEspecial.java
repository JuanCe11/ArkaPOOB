...
/**
 * Su movimiento es inverso al dado por el usuario. Es decir, si el usuario
 * indica un movimiento a la derecha, la plataforma se mover� hacia la
...
public class PlataformaEspecial extends Plataforma {
	...
	public void moveRight(Jugador otroJugador) {
		if (!cambiado) { // En caso de que sea plataforma especial, para que no se quede siempre en
			...
			if (x + getAncho() + VELOCIDAD < limXFinal) { // No se salga por la derecha
				if (x + getAncho() + VELOCIDAD > otroJugador.getX()
						&& x + getAncho() + VELOCIDAD < otroJugador.getX() + otroJugador.getAncho()) { // Si me estrello
																										// con la otra
																										// plataforma
																										// cuando me
																										// muevo hacia
																										// la izquierda
					if (((x - otroJugador.getAncho()) >= limXInicial)
							&& ((otroJugador.getX() + getAncho()) <= limXFinal)) { // Al hacer swap no se salga ninguna
																					// plataforma por los l�mites (Swap
																					// seguro)
						x = otroJugador.getX();
	...
	public void moveLeft(Jugador otroJugador) {
		if (!cambiado) { // En caso de que sea plataforma especial, para que no se quede siempre en
			...
			if (x - VELOCIDAD > limXInicial) { // No se salga por la izquierda
				if (x - VELOCIDAD < otroJugador.getX() + otroJugador.getAncho() && x - VELOCIDAD > otroJugador.getX()) { // Si
																															// me
																															// estrello
																															// con
																															// la
																															...
																															// la
																															// izquierda
					if (((x - getAncho()) >= limXInicial) && ((x + otroJugador.getAncho()) <= limXFinal)) { // Al hacer
																											// swap no
																											// se salga
																											// ninguna
																											// plataforma
